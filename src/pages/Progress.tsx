...
import { Calendar, ChevronDown, TrendingUp, Clock, BarChart2, ChevronLeft, ChevronRight, FileText, Sparkles, AlertCircle, RefreshCw, CheckCircle, Zap } from 'lucide-react';
...
const Progress: React.FC = () => {
  ...
  const [nextUpdateIn, setNextUpdateIn] = useState<number>(120); // 2 minutes in seconds
  ...
  const practiceData = {
    ...
    avgSessionLength: 26,
    weeklyStreak: 5,
    improvedMetrics: ['Pitch Accuracy', 'Breath Control'],
    needsWorkMetrics: ['Rhythm', 'Tone Quality']
  };
  
  const weeklyPractice = [
    { day: 'Mon', minutes: 30, status: 'completed' },
    { day: 'Tue', minutes: 15, status: 'completed' },
    { day: 'Wed', minutes: 20, status: 'completed' },
    { day: 'Thu', minutes: 0, status: 'missed' },
    { day: 'Fri', minutes: 25, status: 'completed' },
    { day: 'Sat', minutes: 10, status: 'completed' },
    { day: 'Sun', minutes: 0, status: 'today' },
  ];
  
  const progressData = [
    { date: 'Aug 1', pitchAccuracy: 65, breathControl: 60, rhythmAccuracy: 70 },
    { date: 'Aug 8', pitchAccuracy: 68, breathControl: 62, rhythmAccuracy: 72 },
    { date: 'Aug 15', pitchAccuracy: 72, breathControl: 65, rhythmAccuracy: 74 },
    { date: 'Aug 22', pitchAccuracy: 75, breathControl: 65, rhythmAccuracy: 78 },
    { date: 'Aug 29', pitchAccuracy: 78, breathControl: 68, rhythmAccuracy: 75 },
  ];
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setNextUpdateIn((prev) => {
        if (prev <= 1) {
          // Trigger new report fetch when timer reaches 0
          fetchReports();
          fetchAgentStatus();
    ...
    >
      ...
      >
        ...
        {showCalendar && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="absolute z-10 bg-gray-900 border border-gray-700 rounded-lg p-4 shadow-xl"
            style={{ top: '100%', right: 0, minWidth: '280px' }}
          >
      ...
      {!loading && !error && getCurrentReport() && (
        ...
        >
          ...
          {/* Insights */}
          {getCurrentReport()?.insights && getCurrentReport()?.insights.length > 0 && (
            <div className="card">
              <h3 className="text-lg font-semibold mb-4 flex items-center">
              ...
              <div className="space-y-3">
                {getCurrentReport()?.insights.map((insight, index) => (
                  <div key={index} className="flex items-start">
                    <div className="w-2 h-2 bg-purple-accent rounded-full mt-2 mr-3 flex-shrink-0"></div>
          ...
          {/* Recommendations */}
          {getCurrentReport()?.recommendations && getCurrentReport()?.recommendations.length > 0 && (
            <div className="card">
              <h3 className="text-lg font-semibold mb-4 flex items-center">
                <TrendingUp size={18} className="mr-2 text-purple-accent" />
                Personalized Recommendations
              </h3>
              <div className="space-y-3">
                {getCurrentReport()?.recommendations.map((recommendation, index) => (
                  <div key={index} className="flex items-start">
      ...
      {!loading && !error && !getCurrentReport() && (
        ...
        >
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-400 mb-2">No Report Available</h3>
            <p className="text-gray-500 mb-4">
              No Fetch AI report is available for {selectedDate.toLocaleDateString()}. 
              Try recording a practice session first!
            </p>
            <div className="text-sm text-gray-600">
              <p>Fetch AI agent runs every 2 minutes to generate reports.</p>
              <p>Next update in {formatTime(nextUpdateIn)}</p>
            </div>
          </div>
        </motion.div>
      )}
